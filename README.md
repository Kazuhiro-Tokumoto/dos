...
`CALCLOCK.ACC` (時計と電卓) を外すと、**GEM のデスクトップが完全に出る。**
...
### まだ残っている: デスクアクセサリを入れると転ぶ

`CALCLOCK.ACC` を置いたままだと、GEM は途中でメモリを壊して止まる。
壊しているのは GEM 自身のブロック転送ルーチン (`GEMVDI` の中の
`rep movsw`) で、そこへ渡っている引数が

```
転送先 4AE5:3B62   転送元 F000:FF67   語数 2A16
```

という出鱈目なものだった。転送元が BIOS の ROM を指し、語数が 1 万語を
超えている。つまり GEM の側のデータが、その前にもう壊れている。

アクセサリ自身が DOS を呼ぶのは 5 回だけで、`AH=19h` (カレントドライブ)
が 1 回と `AH=2Ch` (時刻) が 4 回。オーバーレイの中身は上記のとおり
完全に一致している。
...
| 疑ったもの | 試したこと | 結果 |
|---|---|---|
| 確保したメモリの初期値が違う | `AH=48h` で確保したブロックを 0 で埋めてみる | 変わらない |
| INT 21h の入れ子が深すぎて弾いている | 上限に当たった回数を数える | 0 回 |
| EMS が絡んでいる | CONFIG.SYS を空同然にする | 変わらない |
| `AH=2Ch` の 1/100 秒が常に 0 | BIOS のティックから作るように直す | 変わらない (直し自体は正しいので残した) |
| メモリが足りない | CONFIG.SYS を削って空きを増やす | 変わらない |
| DOS の呼び出しの内容が違う | アクセサリ有り/無しの INT 21h の並びを 1 件ずつ突き合わせる | **714 件が完全に一致**。関数番号も引数も呼び出し元の CS:IP も返り値も同じ |

最後の突き合わせが効いた。GEM が DOS に頼んでいることは、アクセサリの
有無で 1 つも違わない。違いが出るのは `DESKTOP.APP` を起動する
`AH=4Bh` のところで、アクセサリがあるとそこで止まる。

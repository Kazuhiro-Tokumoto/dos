...
### Mpxplay が動いた — 原因は AH=63h だった

以前は「保護モードに入って走り出すところまでは行くが、UI を出すところまで
届かない」状態だった。追ってみると、止まっていたのは **INT 21h AH=63h**
(2 バイト文字の先頭バイト表) の直後。

この機能は `DS:SI` に表の位置を返す決まりで、表は「先頭バイトになりうる
範囲」の組を並べ、0 の組で終わる。2 バイト文字を扱わない実装でも、
**空の表 (0 の組だけ) を返さなければならない**。MYDOS はここを
「何もせず成功を返す」で済ませていたので、`DS:SI` が呼んだ側の値のまま
返っていた。Mpxplay はそこを表だと思って読み始め、たまたま 0 が無いので、
どこまでも範囲の組を拾い続けて戻ってこなかった。

「成功を返しているのだから害はない」と思える種類の手抜きが、いちばん
たちの悪い止まり方をする、という例。

直したところ、Mpxplay 1.68 が完全に動いた。

```
Mpxplay LayerIII  32 kbit 16000Hz  Mono  S16
PSYQUI - ____ (Original Mix)
STILEY: vol.2 (20200123) [Music]
   1 1      1330 7467      0 47 9 / 4 29    82%   10%
```

- MP3 のヘッダを読んで Layer III / 32kbps / 16kHz / モノラルと表示
- ID3 タグからタイトルとアルバムを取り出して表示
- Sound Blaster 16 を見つけて **S16** を選択
- スペクトラムアナライザとプレイリスト編集画面を描画
- 再生位置が進む (0:35 → 0:47 / 4:29)

QEMU の音声出力を WAV に録って中身を確かめたところ、RMS 9,000〜13,000、
1 秒あたりの異なる標本値が 2〜3 万という、まぎれもない音楽の波形だった
(雑音なら値の分布が平らになり、直流なら値が 1 種類しかない)。

**32bit 保護モードの DOS アプリケーションが、MYDOS の上で MP3 を復号して
実際のサウンドカードを鳴らしている。** ここまで来ると、DOS として必要な
ものは一通り揃っていると言ってよさそうに思う。

## EMS (INT 67h) — EMM386.SYS

`CONFIG.SYS` に `DEVICE=\EMM386.SYS` と書くと INT 67h が生え、拡張メモリを
EMS として配れるようになる。

EMS は 8086 が 1MB しか触れなかった時代の抜け道で、考え方はバンク切り替え
そのもの。1MB の中に 64KB の「窓」を 1 つ置き、16KB ずつ 4 枚に区切って、
その裏に用意した何百枚ものページを差し替える。

```
INT 67h AH=41h  → BX に窓のセグメント
INT 67h AH=43h  → BX 枚のページを確保して DX にハンドル
INT 67h AH=44h  → 「窓の AL 枚目に、ハンドル DX の論理ページ BX を出せ」
```

実装したのは 40h 41h 42h 43h 44h 45h 46h 47h 48h 4Bh 4Ch 4Dh 4Eh 51h 58h 59h。

### V86 モードを使っていない

本物の EMM386.EXE は CPU を V86 モードに落としてページングを有効にし、
ページテーブルの書き換えで窓の中身を差し替える。MYDOS のこれは違う。

理由は 2 つある。1 つは、V86 モニタが DOS 本体と同じくらいの規模になること。
割り込みの仮想化、I/O の捕捉、例外の処理を全部書くことになり、EMS を
提供するという目的に見合わない。もう 1 つは、ページングを使わずに 640KB より
上に窓を置く方法が無いこと。チップセットのシャドウ RAM を叩けば開く機械も
あるが、機種ごとに全く違う。当時のメモリマネージャが揃って V86 を選んだのは
まさにこれが理由で、実際 QEMU で C000-EFFF を 16KB ずつ書いて確かめたところ、
書けたのは CC00 と E400-EC00 という飛び飛びの場所だけだった。連続した 64KB は
どこにも取れない。

```
C000=R C400=R C800=R CC00=W D000=R D400=R D800=R DC00=R
E000=R E400=W E800=W EC00=W F000=R F400=R      (W = 書ける)
```

そこで、窓は通常メモリ (ドライバの直後、16KB 境界に揃えた位置) に置き、
ページの実体は XMS に置いて、`AH=44h` のたびに 16KB をコピーしてすげ替える。
窓から追い出すページは必ず書き戻す。API としては完全に正しく動くが、

- 窓が 640KB の中にあるので通常メモリを 64KB 消費する
- 差し替えがデコーダの切り替えではなくコピーなので遅い

という 2 点が本物と違う。プログラムは `AH=41h` で窓の場所を聞く決まりなので、
場所そのものは問題にならない。当時 EMS ボードの無い機械のために配られていた
「EMS エミュレータ」と同じ作りにしてある。

### 検出のされ方

EMS があるかどうかは、`EMMXXXX0` という名前の文字デバイスが開けるかで
調べるのが正式な手順。ドライバはその名前の文字デバイスとして自分を名乗る。

### 書いたものが消えないこと

`tests/emstest.asm` の 9 項目のうち、いちばん大事なのは「窓から追い出して
呼び戻しても中身が残っているか」。ここを外していると、動いているように
見えて書いたものだけが消えるという、いちばん見つけにくい壊れ方になる。
窓 0 にページ 0 を出して書き、ページ 1 を出して追い出し、ページ 0 を
呼び戻して読み返す、という順で確かめている。

作っている最中に出たバグも同じ性格のものだった。`AH=47h` (窓の並びを控える)
が入口で `push ax` していたので、出口の `pop ax` が戻り値の `AH` を
呼び出し時の機能番号 `47h` に戻していた。「エラーコード 47h」という
存在しない値が返り、呼んだ側は失敗と判断する。

## 自分でディスクを作れるようにする — FORMAT と SYS

ここまで、`build/dos.img` はホスト側の `tools/mkimage.py` と mtools が
組み立てていた。MYDOS 自身はディスクを作れず、他のディスクを起動できるように
することもできなかった。それを埋めたのが `FORMAT.COM` と `SYS.COM`。

```
FORMAT B: /V:NEWDISK      ← 1.44MB のフロッピーを FAT12 で作り直す
SYS B:                    ← ブートローダとシステムファイルを移す
```

これで **MYDOS 以外の道具で作った普通のフロッピーが、MYDOS で起動する
ディスクになる**。自動テストの最後で、実際にそのディスクから起動できる
ところまで確かめている。

### FORMAT が BIOS を直接叩く理由

DOS の絶対セクタ書き込み (`INT 26h`) は、そのドライブの諸元を DOS が
知っていることが前提になっている。諸元はブートセクタの BPB から作るので、
まだ何も書かれていないディスクでは作れない。「フォーマットするために
フォーマット済みであることが要る」という循環になるので、当時の FORMAT と
同じく `INT 13h` を直接呼んでいる。

### AH=0Dh に BPB の読み直しをさせた

書き終えたあと、DOS 側は古い姿を信じたままになる。「フォーマットしたのに
空き容量が変わらない」「作ったファイルが読めない」という形で出る。

そこで `AH=0Dh` (ディスクリセット) で、入れ替えのありうるドライブ全部に
「BPB を読み直せ」の印を立てるようにした。次にそのドライブを選んだときに
ブートセクタを読み直す。本物の DOS は、ドライバのメディアチェックが
「変わった」と答えたときに同じことをする。フロッピーの入れ替えを検出する
線は「同じディスクに別の内容を書いた」場合には動かないので、明示的に
頼めるようにしてある。
...
丸ごと上書きすると相手のディスクの構造が分からなくなる。

```
00-0A  移す   ジャンプ命令と OEM 名
0B-3D  残す   BPB と拡張 BPB           ← ここが肝
3E-1FF 移す   ブートコード
```

1.44MB のディスクの Stage1 を 720KB のディスクに丸ごと書いたら、
720KB のディスクが「自分は 2880 セクタある」と名乗り始める。

### IO.SYS の位置を気にしなくていい

本物の MS-DOS は、IO.SYS がルートディレクトリの先頭エントリで、しかも
連続したクラスタに置かれていることを要求した。512 バイトのブートセクタに
FAT を辿るコードが入らなかったため。MYDOS の Stage2 は 8.5KB あって FAT12 を
自分で辿れるので、名前さえ合っていればどこに置かれていてもよい。
だから SYS は普通にファイルとしてコピーするだけで済む。

### ついでに直した DIR

`DIR B:` が「Directory of A:\」と出していた。見出しにカレントディレクトリを
出していたので、別のドライブを指定すると嘘になる。`AH=60h` でパスを
正規化して、実際に並べている場所を出すようにした。

## ハードディスクから起動する

フロッピーができたので、次はハードディスク。手順は 3 つ。

```
FDISK /MBR                      ← マスターブートレコードを書く
FORMAT C: /V:MYDOSHD            ← 区画を作り直す (予約セクタを 18 にする)
SYS C:                          ← ブートローダとシステムファイルを移す
```

これで `-boot c` から MYDOS が立ち上がり、プロンプトが `C:\>` になる。
自動テストの最後で、フロッピーとハードディスクの両方について、
実際に起動できるところまで確かめている。

### 何が足りなかったか

ハードディスクは「ディスクの先頭 = ファイルシステムの先頭」ではない。
先頭 1 セクタは区画の表 (パーティションテーブル) で、その先に区画がある。
そのため次の 3 つが要った。

**1. MBR** — BIOS は LBA 0 を読んで飛ぶだけなので、そこから「起動可能な印の
ついた区画」を探してその先頭セクタを読み込むコードが要る。BIOS が
フロッピーに対してすることを、ソフトウェアでもう一段やる形。自分自身を
0x7C00 から 0x600 へ退避してから次を読むのは、次のブートセクタも 0x7C00 に
置く決まりだから。

**2. 隠しセクタ (BPB の +1Ch)** — 区画の中のブートセクタから見ると、
「LBA 1」は区画の先頭から 1 セクタ目であって、ディスクの 1 セクタ目では
ない。Stage1 と Stage2 の読み込みは、全部これを足した絶対 LBA で行う。
フロッピーではここが 0 なので、同じコードがそのまま動く。

**3. FAT16 と 32bit の LBA** — Stage2 は FAT12 しか読めず、LBA も 16bit
だった。40MB の区画では絶対 LBA が 65535 を超えるし、FAT も FAT16 になる。
ついでに「起動時に FAT を丸ごと読む」のもやめた。FAT12 のフロッピーなら
4.5KB で済むが、FAT16 の 40MB 区画では 40KB になり、置き場所がカーネルの
ロード先 (0x10000) とぶつかる。要る 2 セクタだけをそのつど読むようにした。
2 セクタなのは、FAT12 のエントリがセクタ境界をまたぐことがあるため。

`INT 13h` の拡張読み込み (AH=42h) も足した。区画はトラック境界と揃って
いないので、17 セクタを CHS で一度に読むと BIOS に断られることがある。
使えるかどうかは AH=41h で聞き、駄目なら CHS に落ちる。フロッピーの BIOS は
拡張を持っていないことが多いので、両方要る。

### FORMAT がハードディスクでできること

区画については「すでに入っている BPB をそのまま信じて、割り付けを作り直す」
までしかできない。区画がディスクのどこからどこまでかを知っているのは
パーティションテーブルで、そこは DOS のドライブ文字では指せない場所にある。
そもそも DOS がその区画をドライブとして認識していないと、書き込みの窓口
(`INT 26h`) が使えない。

つまり「一度フォーマットされた区画を作り直す」ことはできるが、何も
書かれていない区画を初期化することはできない。区画を新しく切ったときは、
...
FDISK /D:hh         対象の物理ドライブを選ぶ (既定は 80h)
```

`/MBR` が先頭 446 バイトだけを差し替えるのは、パーティションテーブルを
一緒に書き換えるとディスクの中身が全部行方不明になるため。本物の
`FDISK /MBR` も同じ。区画そのものを作る機能はまだ。

## 完成品の形 — フロッピー 1 枚が母体で、インストーラーを内包する

配布物は `build/dos.img` の **1.44MB フロッピー 1 枚**。OS 本体は約 64KB で、
1 枚の 4.4% しか使っていない。この 1 枚だけで起動でき、そのまま使える。

そのうえで、同じ 1 枚がインストーラーでもある。

```
A:\> SETUP C:
```

これで MYDOS がハードディスクに入り、次からは C: から立ち上がる。
当時の MS-DOS も「起動ディスク 1 枚 → そこから SETUP で HDD にインストール」
という形だったので、そこに合わせてある。

### SETUP がやっていること

```
[1/5] Master boot record            FDISK /MBR
[2/5] Partition layout              FORMAT C: /Q /V:MYDOS
[3/5] Boot loader and system files  SYS C:
[4/5] CONFIG.SYS and AUTOEXEC.BAT   入れる側の姿に合わせて書き下ろす
[5/5] Remaining files               FORMAT SYS FDISK SETUP EMM386 README
```

部品 (`FDISK` / `FORMAT` / `SYS`) は単体でも使えるようにしてあり、SETUP は
それを順に呼ぶ薄い皮でしかない。当時の配布物と同じ分け方。

`CONFIG.SYS` と `AUTOEXEC.BAT` を「写す」のではなく「書き下ろす」のが
細かいところの肝。フロッピー側のものは試験用で、`SHELL=A:\COMMAND.COM` の
ように「A: から起動している」前提の行が入っている。そのまま持っていくと、
次に起動したとき入っていないフロッピーを探しに行く。

### 途中で見つかったもの

**COMMAND.COM が `A:\AUTOEXEC.BAT` を決め打ちしていた。** ハードディスクに
入れた MYDOS は C: から立ち上がるので、自分の AUTOEXEC.BAT が読まれない。
起動したドライブを見るようにした。

**`ECHO OFF` が効いていなかった。** `ECHO` が引数をそのまま出すだけの
実装だったので、`@ECHO OFF` と書くと画面に "OFF" と出ていた。ON/OFF を
状態として持ち、バッチが行を見せるかどうかをそれで決めるようにした
(バッチを抜けるときは ON に戻す)。

**FORMAT が FAT16 でも "FAT12   " と名乗っていた。** ブートセクタのこの
文字列は仕様上ただの飾りで、種別はクラスタ数で決めるのが正しい。とはいえ
嘘を書くと、その文字列を見て判断する道具 (mtools など) がディスクを
読めなくなる。あわせて、FAT16 のクラスタ 1 の終端印が 1 バイト足りず
`00FFh` になっていたのも直した。

## Windows 2.0 の代わりに GEM を動かす

Windows 2.0 は公開されていないので、同時代・同目的でオープンなものに替えた。
**GEM (Digital Research)** — Windows 2.0 と同じ頃の DOS 用グラフィカル
シェルで、当時の直接の競合。1999 年に GPL で公開され、OpenGEM / FreeGEM
として配られている。中の `GEMVDI.EXE` は **1989 年**のバイナリで、
Windows 2.0 と同世代そのもの。

### いまのところ

`GEMVDI` は起動する。640x480 のグラフィックモードに入り、マウスカーソルを
描き、`C:\GEMAPPS\GEMSYS\DESKTOP.APP` を読み込んで `AH=4Bh` で起動する
ところまで行く。そこから先、デスクトップの中身が描かれない。

**同じディスクを FreeDOS 1.3 で起動すると、GEM は完全に動く。**
メニューバー、ウィンドウ、`C:\GEMAPPS\` の 32 個のアイコン、
右側のドライブとごみ箱まで全部出る。つまり OpenGEM の入れ方は正しく、
残っているのは MYDOS 側の互換性の穴だということになる。
FreeDOS のフロッピー版を `-fda`、GEM を入れたディスクを `-hda` で
起動すれば、いつでも「正しい動き」と突き合わせられる。

分かっていること:

- DESKTOP.APP は正しく読み込まれている。走った状態のメモリを吸い出して
  ファイルと 1 バイトずつ突き合わせたところ、違うのはリロケーションを
  当てた 11 語と、プログラム自身が書き換えた 7 バイトだけだった。
- 走り出してはいる。入口 (`loadseg:0000`) にハードウェアブレークを
  置くと止まり、そこから追うと BSS を 0 で埋める初期化を回している。
- **INT 21h を一度も呼ばずに止まる。** FreeDOS では
  `C:\GEMAPPS\` の中身を数えるのに FindFirst/FindNext を使うので、
  その手前で転んでいることになる。
- 割り込みは荒れていない。60 秒で IRQ0 が 1128 回 (18.2Hz ちょうど)、
  他はフロッピーとシリアルだけ。暴走ではなく、待っているか、
  行き先を見失っているかのどちらか。
- CONFIG.SYS を空同然 (FILES/BUFFERS/LASTDRIVE だけ) にしても
  画面は 1 ドットも変わらない。EMS や常駐ドライバは関係ない。

### 追いかけて出てきた 2 つのバグ

デスクトップはまだ出ていないが、追う過程で MYDOS 側のバグが 2 つ出た。
どちらも GEM に限った話ではなく、当時のプログラム一般に効くもの。

**子プロセスが「DOS の中」のまま走っていた。**

`AH=4Bh` は子が終わるまで戻ってこない呼び出しなので、そのままだと親の
1 段が積まれたまま子が走る。子がまた子を起動すれば 2 段、3 段と積み上がり、
入れ子の上限に達したところで DOS が呼び出しを受け付けなくなる (機能番号が
違うと言って返す)。

GEM がまさにこれだった。`COMMAND.COM` → `GEMVDI` → `GEM.EXE` →
`DESKTOP.APP` と 3 段重なり、デスクトップが最初に DOS を呼んだところで
弾かれていた。子へ制御を渡す直前に深さを 0 に戻し、`exec_return` で
書き戻すようにした。

**PSP:0005 の CP/M 形式の呼び出し口が BIOS を指していた。**

CP/M からの移植ものは、DOS を `INT 21h` ではなく「PSP:0005 への near call」
で呼ぶ。機能番号は AH ではなく CL に入れ、使えるのは 0〜24h まで。
PSP:0005 には far call が埋め込んであり、その飛び先が DOS の入口になる。

MYDOS はここを `F000:FEF0` — つまり BIOS の真ん中 — にしていた。形だけ
残したつもりで、実際に呼ばれることを想定していなかった。DESKTOP.APP は
1980 年代のプログラムなので当然これを使い、画面を描き始めたところで
BIOS の中へ飛んで行方不明になっていた。

飛んだ先で何が起きていたかは QEMU の `-debugcon` で SeaBIOS のログを
拾って分かった。BIOS の USB のコードを踏み荒らしたあと、
...
つまり `F000:000C` で例外 5 を出し続けていた。そこにあるのは SeaBIOS の
文字列定数 `"PCI: %s bus = 0x%x"` で、`62 75 73` (= "bus") が
`bound si,[di+0x73]` として実行され、範囲外で落ちる。例外 5 はフォルトなので
`iret` で戻ると同じ命令をまた実行する。永久ループ。

直し方は本物どおりで、`0000:00C0` に DOS の入口への `jmp far` を置き
(INT 30h/31h のベクタの場所をコードとして流用するのも本物と同じ)、
PSP:0005 の far call をそこへ向けた。呼び出し口の中身は FreeDOS の
`kernel/entry.asm` と同じ組み替えをしている。

```
    プログラムが call 5 をした直後のスタック (上が新しい)
        000Ah           ← 埋め込んだ far call が積んだ、意味のない復帰先
        PSP セグメント
        復帰オフセット  ← プログラムの near call が積んだもの

    これを INT 21h と同じ オフセット / セグメント / フラグ に組み替える
```

ついでに、`INT 29h` (高速コンソール出力) と `INT 2Ah`〜`2Eh` も繋いだ。
DOS が押さえている番号なのに BIOS のままにしてあり、`INT 29h` で書いた
文字が黙って消えていた。

回帰テストとして `tests/dosint.asm` に `t_cpm` を足した。PSP:0005 が
...
いまのところ `C:\GEMAPPS\GEMSYS\CALCLOCK.ACC` と `FREEMEM2.ACC` は
消しておく必要がある (理由は下記。アクセサリそのものではなく、
メモリの配置が変わることが効いている)。

### 動いた

この 2 つを直したあと、`CALCLOCK.ACC` (時計と電卓) を外すと
**GEM のデスクトップが完全に出る。**
メニューバー、`C:\GEMAPPS\` のウィンドウとアイコン 32 個、ヘルプの
ウィンドウ、右側のドライブ 4 台とごみ箱まで、FreeDOS で動かしたときと
同じ画面になる。

そこへ辿り着くまでに、MYDOS 側が正しいことを 1 つずつ確かめた。

| 確かめたこと | 結果 |
|---|---|
| GEM の資源ファイル 4 本を 128 バイトずつ読んだ長さと総和 | ホスト側の値と完全一致 |
| `DESKTOP.APP` (65,188 バイト) をメモリ上でファイルと 1 バイトずつ比較 | リロケーション済みの 11 語と、プログラム自身が書いた 7 バイト以外は一致 |
| オーバーレイ (`AH=4Bh AL=3`) で読んだ `CALCLOCK.ACC` の総和 | リロケーションを当てた期待値と一致 (E66B) |
| MCB 連鎖 | GEM が暴走するまでは一貫して健全 |

読み込みも、リロケーションも、メモリ管理も MYDOS 側は合っている。

### まだ残っている: デスクトップが出るかどうかが配置で変わる

`CALCLOCK.ACC` を置いたままだと、GEM は `DESKTOP.APP` を起動したところで
止まる。追いかけた結果、**これはアクセサリの問題ではなかった。**

決め手になったのは、アクセサリを 1 本も置かずにカーネルへ詰め物を入れて
大きさだけを変える実験。
...
| 0 バイト | 出る |
| 256 バイト | 出る |
| 512 バイト | 出ない (「Unable to free memory」) |
| 1024 バイト | 出ない (別の壊れ方) |

アクセサリは 1 本も入っていない。カーネルが数百バイト大きくなり、
アリーナの先頭が数十パラグラフずれただけで結果が変わる。
`CALCLOCK.ACC` (29KB) は、その「悪い配置」を作り出しているにすぎない。

アクセサリ別に見ると次のようになるが、これも配置の違いで説明がつく。

| アクセサリ | 大きさ | 結果 |
|---|---|---|
| なし | — | 出る |
| GROWBOX.ACC | 18,714 | 出る |
| PALETTE.ACC | 26,016 | 出る (資源ファイルが無いという警告は自分で出す) |
| SNAPSHOT.ACC | 13,042 | 同上 |
| CALCLOCK.ACC | 29,184 | 出ない |
| FREEMEM2.ACC | 8,993 | 出ない |

`CALCLOCK.ACC` を `CALCLOC.ACC` に改名しても (8.3 の基本名を 8 文字から
7 文字に縮めても) 結果は変わらないので、名前の長さでもない。

同じディスクを FreeDOS 1.3 で起動すると、どの組み合わせでも
デスクトップは出る。つまり MYDOS 側にまだ何かある。

#### 分かっていること

**アクセサリは DOS を 1 度も呼んでいない。** カーネルの中に INT 21h の
出入りを 16 バイトずつメモリへ溜める仕掛けを入れて (シリアルに出すと
遅くなって症状が変わるため)、失敗する回を丸ごと取った。1,615 件の
記録の呼び出し元はすべて `GEMVDI` / `GEM.EXE` / `COMMAND.COM` で、
アクセサリの載っているセグメントからは 1 件も出ていない。

**INT 21h の入れ子も起きていない。** 同じ記録に入れ子の深さを添えて
数えたが、深さ 2 以上は 0 件。割り込みの中から DOS を呼ばれて壊れている、
という筋ではない。

**`DESKTOP.APP` は正しく読み込まれ、最初の DOS 呼び出しに届く前に死ぬ。**
記録の最後は `AH=4Bh AL=0` (DESKTOP.APP の起動) で、そこから先の記録が
無い。メモリを吸い出して調べたところ、

* 読み込まれた 65,188 バイトはファイル + リロケーションと一致
* PSP の +02 (メモリ上限)、+16 (親)、+2C (環境) すべて正しい
* 環境ブロックにはプログラムのフルパスまで入っている

止まった時点の CPU は `CS=F000 IP=0267` で、無効命令の例外 (INT 06h) が
BIOS の IRET ハンドラへ飛んでは戻る、を繰り返していた。IF=0。

**壊しているのは GEM 自身のブロック転送。** メモリの中身を BIOS の ROM
イメージと突き合わせると、`F000:0000` から 57KB ぶんが GEM の
アプリケーション表 (`DESKTOP`, `SCRENMGR` といった名前が並んでいる場所)
の上に丸ごと複写されていた。GEM の `rep movsw` に出鱈目な引数が
渡っている。つまり GEM のデータはその前にもう壊れている。

**完全に決定的でもない。** `-d int` を付けて (= エミュレーションが遅く
なって) 走らせた 1 回だけ、アクセサリを入れたままデスクトップが完全に
出たことがある。同じ条件の 2 回目は失敗した。

#### 潰した仮説

| 疑ったもの | 試したこと | 結果 |
|---|---|---|
| 確保したメモリの初期値が違う | `AH=48h` のブロックを 0 で埋める | 変わらない |
| INT 21h の入れ子が深すぎて弾いている | 深さを記録して数える | 深さ 2 以上は 0 件 |
| EMS が絡んでいる | CONFIG.SYS を空同然にする | 変わらない |
| `AH=2Ch` の返す値 | 常に 12:00:00.00 を返すようにする | 変わらない |
| メモリが足りない | CONFIG.SYS を削って空きを増やす | 変わらない |
| DOS の呼び出しの内容が違う | アクセサリ有り/無しの INT 21h を 1 件ずつ突き合わせる | アクセサリを読み込む所まで完全に一致 |
| 8.3 の基本名が 8 文字で DTA を溢れさせている | 7 文字に改名する | 変わらない |
| IVT か BIOS データ領域が FreeDOS と違う | 256 本のベクタと BDA を全部突き合わせる | 意味のある違いは INT 22h だけ (下記。直したが症状は変わらず) |

#### 直したもの: EXEC が親の INT 22h を潰していた

突き合わせの途中で見つかった、GEM とは別に効く不具合。

`AH=4Bh` で子を起動するとき、MYDOS は IVT の INT 22h (終了時の戻り先) を
自分の `exec_return` に書き換え、同じ値を子の PSP の +0Ah にも入れていた。
戻ってくる場所としては正しいが、本物の DOS はそんなことをしない。DOS が
子の PSP に写すのは「EXEC を呼んだ時点の INT 22h」で、親が自分で立てた値が
そのまま残る。

FreeDOS の上で `DESKTOP.APP` の PSP を覗くと +0Ah には GEM の番地
(`26CC:F783` — GEM が EXEC を呼んだ次の命令) が入っていた。MYDOS では
DOS 自身の番地が入っていた。当時のプログラムが子の PSP からこの値を読む
ことは珍しくないので、本物と同じにした。戻り先は INT 22h ではなく
カーネル側の入れ子カウンタで覚えるようにしてある。

#### まだ手を付けていない: CONFIG.SYS の `STACKS=`

`STACKS=n,m` は読み取ってはいるが、何もしていない。本物の DOS はこの
指定に従って割り込み用のスタックを用意し、ハードウェア割り込みのたびに
そこへ移してからハンドラを呼ぶ (FreeDOS も INT 02h/08h〜0Eh/70h〜77h に
同じ仕掛けを入れている)。実装して試してみたが、上記のとおり GEM の
成否がカーネルの大きさで変わってしまうため、効いているのかどうかを
切り分けられなかった。カーネルが 640 バイト大きくなるぶんだけ配置が
ずれ、アクセサリ無しでもデスクトップが出なくなる。配置の依存を先に
潰さないと評価できないので、いったん入れていない。

### 途中で見つかった: メモリのアリーナが EBDA を踏んでいた

追う過程でこれが出た。MYDOS はコンベンショナルメモリの上限を
`0xA000` (640KB) に決め打ちしていた。ところが BIOS は **EBDA
(拡張 BIOS データ領域)** を 640KB のすぐ下に置くことがあり、そこには
PS/2 マウスの状態やハードディスクの諸元表が入っている。QEMU の
SeaBIOS もそうで、e820 は

```
0x00000 - 0x9FC00 = RAM
0x9FC00 - 0xA0000 = 予約   ← EBDA
```

と申告している。FreeDOS の MCB 連鎖は `0x9FC0` で止まっていたが、
MYDOS は `0xA000` まで伸ばしていた。上のほうまで使うプログラムは、
そのまま EBDA を踏む。踏まれた BIOS はそのあと出鱈目な値を返すように
なる。

BIOS は `0040:0013` に「使える大きさ (KB)」を書いてあり、EBDA がある
機械では 640 ではなく 639 になっている。そこから上限を取るようにした。
回帰テストとして `tests/dosint.asm` に `t_memtop` を足し、連鎖の末尾が
BIOS の申告と一致することを見るようにした。

(この修正でアクセサリが直ったわけではないが、当時のプログラムには
上限まで確保して使うものが珍しくないので、これは踏んではいけない類の
バグだった。)

暴走している瞬間の呼び出し元をスタックから辿ろうとしたが、その時点で
すでに BP の連鎖が壊れており (フレームのはずの場所に 32bit の push の
命令が並んでいた)、これは最初の異常ではなく後始末の側だった。
最初にどこで GEM の状態が壊れるかは、まだ掴めていない。

### 途中で見つかって直したもの

**`AH=58h` (メモリの割り当て方) の下位機能が食い違っていた。**
この機能は 4 つに分かれている。

```
AL=00h  いまの方針を返す
AL=01h  方針を決める        (BL = 方針)
AL=02h  UMB を連鎖に入れているか
AL=03h  UMB を連鎖に入れる  (BX = 0/1)
```

MYDOS は「AL=0 なら取得、それ以外は設定」という 2 分岐にしていたので、
`AL=03h` (UMB を繋ぐ) が方針を書き換え、`AL=02h` が誤りとして返っていた。
さらに方針の値を 0〜2 に限っていたのも間違いで、DOS 5.0 から上位ビットで
「上 (UMB) を先に探す」を表す `40h`〜`42h`、`80h`〜`82h` が増えている。
UMB へ載ろうとするプログラムは `41h` や `81h` を指定してくるので、
そこで全部弾かれていた。

**MCB の解放が、連鎖にないブロックを受け入れていた。**
`ES-1` の 1 バイトが `'M'` か `'Z'` に見えるかどうかだけで判断していたので、
プログラムが渡してきた出鱈目なセグメントでも、そこがたまたまその値なら
「正しいブロック」として持ち主を 0 にしてしまう。誰かのデータの真ん中に
空きブロックがあることになり、次の確保でアリーナごと壊れる。
連鎖を頭から辿って一致するものがあるかまで見るようにした。

### `.EXE` に渡すメモリを、バイト数で数えていた

GEM は動作条件にマウスを挙げている。マウスドライバ (CuteMouse) を常駐
させると、そのあとの確保がすべて「MCB が壊れている」で返るようになった。
壊れているのは CuteMouse のブロックのすぐ後ろにあるはずの MCB で、
そこには CuteMouse のものらしきデータが載っていた。

QEMU の gdb スタブでその番地に書き込み監視を仕掛けたところ、
書いていたのは `SS = PSP`, `SP = ブロックの末尾` で走っている
CuteMouse 自身だった。つまり MYDOS が渡したブロックが、CuteMouse が
必要とする量より小さい。

原因は `.EXE` の大きさの数え方だった。MYDOS はこう計算していた。

```
イメージのバイト数 = ページ数 * 512 - (最終ページの半端なぶん) - ヘッダ長
確保するパラグラフ数 = そのバイト数を 16 で割って切り上げ + 16 + min_alloc
```

本物の DOS はこうではない。**バイト数ではなくページ数から数える。**

```
確保するパラグラフ数 = ページ数 * 32 - ヘッダのパラグラフ数 + 16 + min_alloc
```

最終ページが半端でも切り上げる。CuteMouse は 10 ページ・最終ページ
240 バイトなので、この差は 17 パラグラフ。ちょうどその分だけブロックが
小さくなり、`PSP:0002` が本来より下を指していた。

これが効くのは、そこを上限にして自分のスタックを置くプログラムがあるため。
当時の常駐ソフトの定石で、`SS = PSP`, `SP = ([PSP:0002] - PSP) * 16` と
置く。ブロックが 17 パラグラフ足りなければ、スタックはそのまま次の MCB を
踏む。踏まれた MCB は次の確保で連鎖の切れ目になり、そこから先の
`AH=48h` も `AH=4Bh` も「MCB が壊れている」で返る。症状 (常駐後に EXEC が
失敗する) と原因 (ローダの掛け算) が遠いのは、そのため。

FreeDOS のカーネル (`kernel/task.c`) も同じ計算をしている。

```c
image_size = (ExeHeader.exPages << 5) - ExeHeader.exHeaderSize;
```

16bit で計算しているのも意図的で、ページ数が 2048 以上のときに上位が落ちる。
1MB を超えるイメージを持つ NE の DOS スタブは「1MB で割った余りだけ読む」
作りになっているので、この桁あふれがそのまま正しい動きになる。MYDOS も
それに合わせた。

ついでに、同じ場所にあるもう一つの作法も入れた。**`min_alloc` と
`max_alloc` がどちらも 0 のときは、DOS は空きをすべて渡し、その中の
いちばん上にイメージを置く。** RBIL に載っている挙動で、これを前提に
しているプログラムが実在する。

回帰テストとして `tests/exetop.asm` を足した。本体をちょうど 1040 バイトに
揃えてあり、ヘッダと合わせて 3 ページ (最終ページは 48 バイトだけ使用)
になる。`min_alloc = max_alloc = 0x20` なので渡される量が一意に決まり、
正しければ 142 パラグラフ、バイト数で数えると 113 パラグラフになる。
プログラムは `PSP:0002` を読んでその値を突き合わせる。

これで CuteMouse は正しく常駐し、そのあとの `.EXE` の起動も通るように
なった。
...
- **UMB** (`DOS=UMB`) — 640KB と 1MB の間の「穴」に RAM を貼るには V86
  ...
  だけ書いたときの本物の振る舞いと同じ)
- **EMS** (LIM 4.0) — **`DEVICE=\EMM386.SYS` で使えるようにした** (下記)
...
- **`FDISK`** — パーティションを切る側。`FORMAT` はフロッピー向けに作った (下記)

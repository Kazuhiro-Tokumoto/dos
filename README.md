...
`FORMAT` はフロッピー (A: / B:) 専用。ハードディスクはパーティションを切る
話 (FDISK) が先に来るので扱わない。`SYS` のほうはドライブを選ばないが、
ハードディスクから起動するには MBR とパーティション先頭の考慮 (BPB の
hidden_secs) が Stage1/Stage2 側に要る。そこはまだ。

...
**解決済み**: 長いこと「XMS のブロック転送 (`0Bh`) を呼んだあと C: を
触ると壊れる／再起動する」という不具合が残っていて、そのために自動テストでは
XMS の試験を最後に回していた。原因を追ったところ、**カーネルではなく
テストプログラム側のバグ**だった。

`tests/xmstest.asm` が、転送先バッファを 0xEE で汚してから読み返す、という
確認をしている。その `rep stosb` の前に `ES` を入れ直していなかった。
直前に `INT 2Fh AX=4310h` (XMS の入口を聞く) を呼んでいるので、`ES` には
**カーネルのセグメント**が入ったままになっている。結果、0xEE が 256 バイト、
カーネルのコードの上に書き込まれていた。

たちが悪いのは、書いた瞬間には何も起きないこと。潰したのは C: を触るときに
しか通らない経路だったので、A: の操作は最後まで正常に動き、しばらくしてから
別ドライブに触った瞬間に機械ごと落ちる。「XMS が壊す」ように見えていたが、
XMS は何も悪くなかった。

調べる過程で `xms_block_move` に本物のバグも 3 つ見つかったので、そちらも
直してある。

- 転送のために開けた **A20 を開けっぱなし**で返していた (HIMEM.SYS は
  前後で元に戻す)
- **GDTR を差し替えたまま**返していた。この先 BIOS や常駐ソフトが
  プロテクトモードへ出入りするとき、自分が置いたつもりの GDT ではなく
  MYDOS の 2 エントリしかない表を見ることになる
- キーボードコントローラ経由で A20 を開け閉めするとき、出力ポートの
  **bit0 (CPU のリセット線)** を読めた値のまま書き戻していた。0 が返って
  きた瞬間に機械が再起動する

自動テストには「XMSTEST のあとで C: の DIR が通ること」を 1 件足した。

...
ホストが要る。MYDOS はこれを自前では持っていないので、フリーの
**CWSDPMI** (DJGPP のもの、Charles W Sandmann 作) を C: に置いて試した。
    ...
    C:\>DPMICHK
      AX (0 = present) = 0000
      DPMI HOST PRESENT
      DX version       = 005A      (DPMI 0.90)
      BX flags         = 0001      (32bit クライアントを扱える)
      ES:DI entry      = 21CD:117C

**ここまでは通る。** CWSDPMI は本物のリンカが吐いた 16bit EXE
(512 バイトヘッダ、リロケーション 25 個、SS≠0) で、MYDOS の上で起動し、
常駐し、INT 2Fh AX=1687h に正しく答えるところまで動く。常駐は一度きりで、
`CWSDPMI -u` 一回で消えることも確かめた。

**その先はまだ通らない。** DJGPP で作られた実物のプログラム
(`djecho.exe`) を走らせると、保護モードに入って要求ページングの
ページフォルト (CR2=0x00410000) まで進んだあと、`0060:0317` — つまり
MYDOS のカーネル自身 — で無効オペコードを無限に踏む。そのときメモリ上の
カーネルは 32bit のコードで丸ごと書き潰されている。

分かっていること:

* 書き潰しは CWSDPMI が常駐した時点ではまだ起きていない。DJGPP
  プログラムを走らせている最中に起きる。
* 潰した中身は EXE ファイルの中身ではない (バイト列がどのファイルにも
  無い)。CWSDPMI が実行時に作ったものと思われる。
* 線形アドレスに掛けたウォッチポイントには引っかからない。CWSDPMI が
  ページングを有効にしたあとの書き込みなので、線形と物理がずれている。
* MYDOS の XMS が返す数字は正常 (ロックしたブロックの物理アドレスは
  0x00110000)。

原因は未特定。DPMI ホストと同居するために MYDOS 側に何が足りないのかを
まだ掴めていない。`tests/dpmichk.asm` と `tests/dpmirun.asm` は、その
調査に使った道具として残してある (DPMI ホストを同梱していないので
自動テストには入れていない)。

この調査の途中で、INT 2Fh のハンドラが「知らない機能に対して AL=0 を
書いて返す」という間違いをしていたのが見つかった。INT 2Fh は呼ぶ側が
AL=0 を入れて呼び、常駐したものがあれば AL=FFh を返す、という導入確認の
待ち合わせ場所なので、DOS が勝手に AL を触ってはいけない。AL に引数を
載せる機能 (DPMI の 1687h など) では返り値そのものが化ける。これは
直してあり、`tests/xmstest.asm` に回帰テストを足した。

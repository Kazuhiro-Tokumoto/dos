...
### まだ残っている: CuteMouse を常駐させると EXEC が失敗する

GEM は動作条件にマウスを挙げている。マウスドライバ (CuteMouse) を常駐
させると、そのあとの `AH=4Bh` が失敗するようになる。追ったところ、
CuteMouse は「自分のブロックを解放 → 方針を上優先に → 常駐ぶんを確保し直す」
という *load high* の常套手段を踏むのだが、その確保が
「MCB が壊れている」で返ってくる。壊れているのは CuteMouse のブロックの
すぐ後ろにあるはずの MCB で、そこには CuteMouse のコードらしきものが
載っている。EXE の確保量の計算か、リロケーションの当て方のどちらかが
疑わしいが、まだ特定できていない。MYDOS 自身の TSR (`tsrtest.com`) では
再現しないので、`.COM` ではなく `.EXE` のローダ側の問題と思われる。

...
この 2 つを直すと、例外 5 のループは消え、`.too_deep` も踏まなくなる。
それでもデスクトップは描かれない。いまは `GEM.EXE` のブロックのすぐ後ろの
MCB (自分のブロックの 1 パラグラフ先) が壊れており、`DESKTOP.APP` を
起動しようとしたところで確保が「MCB が壊れている」で返る。CuteMouse の
ときと同じ形なので、まだどこかで渡す量が足りていない可能性がある。

GEM のアプリは DOS ではなく GEM の AES/VDI を `INT EFh` で呼ぶので、
INT 21h のトレースには最初から映らない。追うときは QEMU の
`-d exec` を、`GEMVDI` が `DESKTOP.APP` の入口にハードウェアブレークで
止まったところからモニタの `log exec` で始めると、量が現実的になる。

...
追ってみると、`AH=4Bh` を最後に **INT 21h の呼び出しが完全に止まる**。
これは行き詰まりの証拠にはならない。GEM のアプリは DOS ではなく GEM の
AES/VDI を `INT EFh` で呼ぶので、INT 21h のトレースには最初から映らない。
画面に出ていないだけで走っている可能性と、DESKTOP.APP が転んでいる
可能性の両方がある。ここは未解決。
